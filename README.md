...
7. **Cross-repository statistical aggregation**
   Per-repository summaries are aggregated across QC-passing repositories to compute cross-repo statistics
   (mean, median, min/max, standard deviation, count) for defect categories and R/Rmd touch rates.
...
- `/scripts`
  ...
  - `batch_qc_all.py` (updated) — includes stricter thresholds and produces final PASS/FAIL used in thesis analysis


- `/data_bug`  
  Contains one folder per repository. Each repository folder includes:
  - `<repo>_bug_commits.csv` — raw bug-keyword commit data
  - `<repo>_bug_commits_classified.csv` — commit-level classification (10 defect categories)
  - Optional example files:
    - `<repo>_bug_commits_examples.csv`
    - `<repo>_bug_commits_examples_appendix.pdf`
  - Per-repo visualizations:
    - `<repo>_bug_commits_category_counts.png`
  - Per-repo summary files (naming may begin with either `_bug_commits_` or `_bug_commits_classified_`):
    - `*_category_percentages.csv`
    - `*_rmd_touch_by_category.csv`
    - `*_r_touch_by_category.csv`
    - `*_top_paths.csv`
    - These files are generated by `summarize_repo.py` or `batch_summarize_repos.py`.
    
     Repositories failing QC remain in `/data_bug` but are excluded from all cross-repository analyses.


- `/analysis`  
  Contains cross-repo outputs:
   - `qc_summary.csv` — final QC table showing PASS/FAIL status for all repositories (53 PASS, 4 FAIL)
   - `category_stats.csv` — cross-repository statistics (mean, median, std, min, max, count) for defect categories
   - `touch_stats_by_category.csv` — aggregated R / Rmd touch rates by defect category across QC-passing repositories
...
- **Reproducibility note**:  
  All experiments were executed using a fixed Python environment during the final batch run.  
  Results reported in the thesis are fully reproducible using the scripts and CSV artifacts committed to this repository, without requiring access to GitHub APIs.

- **Optional (analysis-only) dependencies**:  
  Some plotting and aggregation scripts may additionally use:
  - `numpy`
  - `seaborn` (for exploratory plots only)

  These are not required to reproduce the core classification or QC results.
...
- **Scope and safety**:
  The GitHub token is required only for `fetch_bug_commits_all.py` to avoid API rate limits.
  It is never written to disk, logged, or included in any output CSV files.

- **Offline reproducibility**:
  All classification, QC, summarization, and analysis steps operate entirely on the
  fetched CSV artifacts. Once the fetch step is completed, the full pipeline can be
  rerun offline without any GitHub access.

- **Final dataset note**:
  The final thesis results are based on a completed batch run, and re-fetching commits
  is not necessary to reproduce any tables, figures, or statistics reported.
...
**QC failure interpretation (important):**
Repositories may fail QC primarily due to structural characteristics rather than
classifier malfunction. In particular, repositories dominated by:
- documentation-only changes (PDF, HTML, LaTeX, XML),
- large auto-generated artifacts,
- or bulk content updates with minimal code diffs

tend to exhibit lower coverage and higher low-confidence rates, even when commits
are genuine bug fixes.

For example, repositories such as:
- `CDCgov/MIRA`
- `rjournal/rjournal.github.io`

failed QC due to a high proportion of formatting, publishing, or conversion-related
commits with limited diff-level semantic signals. These repositories are analyzed
qualitatively and excluded from the main quantitative analysis to preserve result
reliability.

Low-confidence commits (score ≤ 2) are not errors by default.
They typically arise from:
- minimal diffs (e.g., one-line fixes),
- file-only evidence without semantic cues,
- or documentation / publishing corrections.

Manual inspection of representative low-confidence samples confirms that
these commits are often valid bug fixes, but lack sufficient diff-level
evidence to be classified with high confidence using automated rules.
...
Touch flags (`touches_r`, `touches_rmd`) are recorded independently of
defect classification and are used exclusively for analytical purposes.

A commit may:
- touch R Markdown files without being a rendering defect,
- or fix logic errors inside code chunks within `.Rmd` files.

Therefore, touch rates are interpreted as indicators of workflow context,
not as proxies for defect category.
...
This confirms that the reported cross-repository defect distributions are driven by diff- and path-level evidence rather than heuristic post-processing decisions.
...
### Cross-repo outputs
The following analysis-ready outputs were generated for the QC-passing set:

- `analysis/cross_repo_category_percentages.csv`  
  Per-repository defect-category percentages (used as the basis for cross-repo aggregation).

- `analysis/category_stats.csv`  
  Cross-repository statistics for each defect category (mean, median, min/max, standard deviation, count), computed across QC-passing repositories.

- `analysis/touch_stats_by_category.csv`  
  Cross-repository statistics of `.R` and `.Rmd` touch rates by defect category (mean, median, min/max, standard deviation, count), computed across QC-passing repositories.
...
For replication, cite the repository commit/tag corresponding to the final batch run and the QC summary in `analysis/qc_summary.csv`.
Key analysis artifacts include `analysis/cross_repo_category_percentages.csv`, `analysis/category_stats.csv`, and `analysis/touch_stats_by_category.csv`.
