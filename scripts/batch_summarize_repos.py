#!/usr/bin/env python3
# batch_summarize_repos.py
#
# Usage:
#   python scripts/batch_summarize_repos.py --dir data_bug
#
# It will:
#   - For each subdirectory under data_bug/ (assumed one per repo):
#       * Look for "*_bug_commits_classified.csv"
#       * If found, summarize it (category %, R/Rmd touch, top paths)
#       * If not found, record the repo as a failure
#   - At the end, write summarize_failed_repos.csv in data_bug/
#     with a list of repos that could not be summarized and the reason.
#
# It does NOT try to read any *_category_percentages.csv beforehand,
# so missing summary files will not crash the script.

import os
import sys
import argparse
import pandas as pd
from collections import Counter

# --- helpers (from summarize_repo.py) ---

def detect(df, *cands):
    lower = {c.lower(): c for c in df.columns}
    for c in cands:
        if c.lower() in lower:
            return lower[c.lower()]
    return None

def infer_rmd_touch(row, col_files):
    """If touches_rmd is missing, infer it from filenames."""
    if col_files is None:
        return False
    val = str(row[col_files] or "")
    toks = [t.strip().lower() for t in val.split(";") if t.strip()]
    if not toks:
        return False
    RMD_HINTS = (".rmd", ".qmd", "_site.yml", "_output.yml", "bookdown.yml")
    return any(any(tok.endswith(h) or h in tok for h in RMD_HINTS) for tok in toks)

def pct_table(d, col_cat):
    c = d[col_cat].value_counts(dropna=False).rename_axis('bug_category').reset_index(name='count')
    c['percent'] = (c['count'] / len(d) * 100).round(1)
    return c.sort_values('count', ascending=False).reset_index(drop=True)

# --- per-repo summarization ---

def summarize_one_repo(in_csv: str):
    df = pd.read_csv(in_csv)

    # Column detection
    col_cat   = detect(df, "bug_category","category","label")
    col_score = detect(df, "category_score","score")
    col_touch = detect(df, "touches_rmd","touch_rmd","rmd")
    col_files = detect(df, "filenames","files","paths")
    col_touch_r = detect(df, "touches_r", "touch_r")

    if col_cat is None:
        raise RuntimeError("Could not find bug_category column (aliases: bug_category/category/label).")

    # Ensure strings for safe ops
    if col_files and col_files in df:
        df[col_files] = df[col_files].fillna("").astype(str)

    # 1) Category percentages
    pct = pct_table(df, col_cat)

    # 2) Rmd touch rates by category
    if col_touch and col_touch in df.columns:
        s = df[col_touch]
        if s.dtype == object:
            s = s.astype(str).str.strip().str.lower().isin(["true","t","1","yes","y"])
        else:
            s = s.astype(bool)
        df["_touch_rmd"] = s
    else:
        df["_touch_rmd"] = df.apply(lambda r: infer_rmd_touch(r, col_files), axis=1)

    touch_rmd = (
        df.groupby(col_cat)["_touch_rmd"]
          .mean().mul(100).round(1)
          .rename("touches_rmd_%")
          .reset_index()
          .sort_values("touches_rmd_%", ascending=False)
    )

    # 3) Top paths (from filenames)
    top_paths = pd.DataFrame(columns=["path","count"])
    if col_files and col_files in df.columns:
        all_files = []
        for x in df[col_files]:
            if not x:
                continue
            all_files.extend([p.strip() for p in str(x).split(";") if p.strip()])
        if all_files:
            cnt = Counter(all_files).most_common(15)
            top_paths = pd.DataFrame(cnt, columns=["path","count"])

    # 4) R touch rates by category
    if col_touch_r and col_touch_r in df.columns:
        s_r = df[col_touch_r]
        if s_r.dtype == object:
            s_r = s_r.astype(str).str.strip().str.lower().isin(["true","t","1","yes","y"])
        else:
            s_r = s_r.astype(bool)
        df["_touch_r"] = s_r
    else:
        df["_touch_r"] = False
        if col_files and col_files in df.columns:
            df["_touch_r"] = df[col_files].astype(str).str.lower().str.contains(r"\.r(;|$)", regex=True)

    touch_r = (
        df.groupby(col_cat)["_touch_r"]
          .mean().mul(100).round(1)
          .rename("touches_r_%")
          .reset_index()
          .sort_values("touches_r_%", ascending=False)
    )

    # Save next to input
    base, _ = os.path.splitext(in_csv)
    out_touch_r = base + "_r_touch_by_category.csv"
    out_pct     = base + "_category_percentages.csv"
    out_touch   = base + "_rmd_touch_by_category.csv"
    out_paths   = base + "_top_paths.csv"

    touch_r.to_csv(out_touch_r, index=False)
    pct.to_csv(out_pct, index=False)
    touch_rmd.to_csv(out_touch, index=False)
    if not top_paths.empty:
        top_paths.to_csv(out_paths, index=False)

    # Console summary (optional)
    print(f"\n[SUMMARY] {in_csv}")
    print(pct)
    print("\nTouches .Rmd by category (%):")
    print(touch_rmd)
    if not top_paths.empty:
        print("\nTop touched paths:")
        print(top_paths)
    else:
        print("\nTop touched paths: (no filenames column found or empty)")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--dir", required=True,
        help="Base folder containing per-repo data (e.g. data_bug)"
    )
    args = parser.parse_args()

    base_dir = args.dir
    if not os.path.isdir(base_dir):
        print(f"[ERROR] Directory not found: {base_dir}", file=sys.stderr)
        sys.exit(1)

    failures = []  # list of dicts: {repo, path, reason}

    # Each subdirectory of base_dir is treated as one repo folder
    subdirs = [d for d in os.listdir(base_dir)
               if os.path.isdir(os.path.join(base_dir, d))]

    if not subdirs:
        print(f"[ERROR] No subdirectories (repos) found under {base_dir}", file=sys.stderr)
        sys.exit(1)

    print(f"Found {len(subdirs)} repo directories under {base_dir}.")

    for repo_dir_name in sorted(subdirs):
        repo_path = os.path.join(base_dir, repo_dir_name)
        print(f"\n[PROCESSING REPO] {repo_dir_name}")

        # Look for *_bug_commits_classified.csv inside this repo folder
        classified_files = [
            os.path.join(repo_path, f)
            for f in os.listdir(repo_path)
            if f.endswith("_bug_commits_classified.csv")
        ]

        if not classified_files:
            print(f"[WARN] No *_bug_commits_classified.csv found in {repo_path}")
            failures.append({
                "repo": repo_dir_name,
                "path": repo_path,
                "reason": "no *_bug_commits_classified.csv found"
            })
            continue

        # If multiple match, summarize each
        for in_csv in sorted(classified_files):
            try:
                print(f"[FILE] {in_csv}")
                summarize_one_repo(in_csv)
            except Exception as e:
                print(f"[ERROR] Failed to summarize {in_csv}: {e}", file=sys.stderr)
                failures.append({
                    "repo": repo_dir_name,
                    "path": in_csv,
                    "reason": f"exception during summarization: {e}"
                })
                # Skip to next file / repo
                continue

    # Write failures log
    if failures:
        fail_df = pd.DataFrame(failures)
        out_fail = os.path.join(base_dir, "summarize_failed_repos.csv")
        fail_df.to_csv(out_fail, index=False)
        print(f"\n[INFO] Some repos could not be summarized. Details written to:\n  {out_fail}")
    else:
        print("\n[INFO] All repos summarized successfully; no failures log created.")

    print("\nDone. Per-repo summary files have been written next to each classified CSV.")

if __name__ == "__main__":
    main()
